<form action="{{ $action }}" id="formGallery"></form>
@if(!empty($dataE))
<input type="hidden" name="id" value="{{ $dataE->id }}" form="formGallery">
@endif
@csrf
...
<div class="form-group">
    <div style="width:100%; height: 185px; text-align: center; position: relative" id="image">
        <img width="100%" height="100%" id="preview_image"
            src="{{ (!empty($dataE->gambar) ? 'storage/master-data/gallery/uploads/' . $dataE->gambar : asset('images/noimage.jpg') ) }}" />
    </div>
    <div class="row mt-5">
    ...
    <input type="file" id="file" name="gambar" class="hide" form="formGallery" />
    <input type="hidden" id="file_name" form="formGallery" @if(!empty($dataE)) name="old_img"
...
<div class="form-group">
    ...
    <div class="input-group input-group-sm date">
        ...
        <textarea name="keterangan" class="form-control add-style" placeholder="Keterangan..." style="height:70px;"
            form="formGallery">{{ (!empty($dataE) ? $dataE->keterangan : null) }}</textarea>
    </div>
</div>
