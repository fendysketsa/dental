...
function load_formAdd() {
    ...
    cont.load(base_url + "/rooms/add", function (e, s, f) {
        if (s == "error") {
...
function form_attribut(id) {
    ...
    var images = $(".input-template").data("images");

    let preloaded = images;

    $(".input-template").imageUploader({
        preloaded: preloaded,
    ...
    $(".image-uploader").find("input").attr("form", "formRoom");
}
...
function load_data() {
    ...
    $(".display-future").addClass("blocking-content");
    cont.load(base_url + "/rooms/data", function (e, s, f) {
        if (s == "error") {
...
function submit(page) {
    $("form#formRoom").submit(function (e) {
        e.preventDefault();
