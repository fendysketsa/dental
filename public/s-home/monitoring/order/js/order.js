    ...
    $('tbody').delegate('.edit', 'click', function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('.modal-title').html('<em class="fa fa-pencil-square-o"></em> Form Pendaftaran')
                load_formLeft();
                select_members();
                ...
                setTimeout(function () {
                    setTimeout(function () {
                        $(".f-new-member").html('<div class="row"><div class="text-center"><em class="fa fa-spin fa-spinner"></em> loading...</div></div>');
                        ...
                        setTimeout(() => {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    setTimeout(() => {
                                        $('select[name=sno_member]').val(inputId.data('member-id'));
                                        $('select[name=sno_member]').trigger('change.select2');
                                        setTimeout(() => {
                        ...
                        for (var rL = 1; rL <= inputId.data('layanan').length; rL++) {
                        ...
                        $("input[name=jumlah_orang]").val(inputId.data('jum_org')).trigger('change')
                        $("input[name=dp]").val(inputId.data('dp')).trigger('change')
                        $(".price-full").html(inputId.data('total-biaya')).trigger('change')


                        if (inputId.data('reservasi') !== "") {
                            var attr_ = $("button#show");
                            attr_.attr('id', 'hide')
                            attr_.find('em').removeClass('fa-question-circle').trigger('change').addClass('fa-check-circle')
                            attr_.removeClass('btn-default').trigger('change').addClass('btn-success')
                            ...
                            $("input[name=jam_reservasi]").timepicker({
                                showInputs: false,
                                showMeridian: false,
                                timeFormat: 'HH:mm',
                                step: 15
                            }).val(formatTime(waktu[1])).trigger('change');
                            select_('lokasi');
                            ...
                            $("input[name=tgl_reservasi]").val(formatDate(waktu[0])).trigger('change')
                            setTimeout(function () {
                                $("select[name=lokasi_reservasi]").val(inputId.data('lokasi')).trigger('change')
                            }, 700);
                        }
                    }, 500);
                }, 500);
...
function rePrint(events, dataTrans, onsave) {
    ...
    printer.open().then(function () {
        ...
        printer.align('center').text("-------------------------------").text('WORK ORDER')
            .feed(1)
            ...
            .align('left').text('Operator' + sprintf('%4s', ': ') + sprintf('%20s', operator))
            .align('center').text("-------------------------------");

        if (dataTrans.layanan.length > 0) {
            for (var lyn = 0; lyn < dataTrans.layanan.length; lyn++) {
                printer
                    .align('left').text(dataTrans.layanan[lyn].kategori.substring(0, 28))
                    .align('left').text(
                        sprintf('%s', ' 1 X ' + dataTrans.layanan[lyn].layanan.substring(0, 25) + '/' +
                            (dataTrans.layanan[lyn].terapis ? dataTrans.layanan[lyn].terapis.substring(0, 8) : '-')
                        )
                    );
            }
        }
        ...
        if (dataTrans.paket.length > 0) {
            for (var pketl = 0; pketl < dataTrans.paket.length; pketl++) {
                if (dataTrans.paket[pketl].paket) {
                    jumPaket += 1;
                    printer.align('left').text(dataTrans.paket[pketl].paket.substring(0, 28));
                }
                printer.align('left').text(
                    ' - ' + sprintf('%s', dataTrans.paket[pketl].layanan.substring(0, 25) + '/' +
                        (dataTrans.paket[pketl].terapis ? dataTrans.paket[pketl].terapis.substring(0, 8) : '-')
                    )
                );
            }
        }
        ...
        printer
            .align('center').text("-------------------------------").text('Selamat bekerja, Semangat!')
            .align('center').text('')
            .cut()
            .print()
...
function toSendPembayaran(event, onsave) {
    var target = !onsave ? event.data('route') : (onsave == 'send-pembayaran' ? event.data('route-send-pembayaran') : event.data('route-on'));
    var idCetak = !onsave ? event.data('id-cetak') : (onsave == 'send-pembayaran' ? event.data('id-send-pembayaran') : event.data('id-cetak-on'));
    var tables = event.closest('table');
...
function input_member() {
    ...
    html += `<input type="text" name="ino_member" value="` + DatAuto + `" class="form-control input-sm" placeholder="No Member..." form="formRegistrasi">`;
    return html;
}
...
function f_member(data) {
    ...
    html += `<div class="form-group">
                ...
                <div class="input-group input-group-sm date">
                    ...
                    <input type="text" name="nama" value="` + (data.nama ? data.nama : '') + `" class="form-control" placeholder="Nama..." form="formRegistrasi">
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="email" name="email" value="` + (data.email ? data.email : '') + `" class="form-control" placeholder="Email..." form="formRegistrasi">
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="input-group input-group-sm date">
                            ...
                            <input type="text" name="telepon" value="` + (data.telepon ? data.telepon : '') + `" class="form-control" placeholder="Telepon..." form="formRegistrasi">
                        </div>
                    </div>
                </div>
            </div>
            <div class="form-group input-group-sm">
                <label>Alamat:</label>
                    ...
                    <textarea name="alamat" cols="15" rows="4" class="form-control input-sm add-style" form="formRegistrasi" placeholder="Alamat...">` + (data.alamat ? data.alamat : '') + `</textarea>
                </div>
            </div>`;
    return html;
}
...
function getHarga(table, el, i) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function (data) {
            $(".price-layanan-unique").attr('data-price-layanan-new-' + i, data.harga)
        }
    });
}
...
function form_attribut_right() {
    ...
    $("button.collap-paket").on('click', function (e) {
        ...
        if (exp == 'true') {
            $(this).find('em').removeClass('fa-minus').addClass('fa-plus').trigger('change')
        } else {
            $(this).find('em').removeClass('fa-plus').addClass('fa-minus').trigger('change')
    ...
    $("table tbody.load-row-layanan").delegate('tr > td > em.remove-row-layanan', 'click', function (e) {
        $(this).parents("tr").html('<td colspan="4" class="text-center"><em class="fa fa-spinner fa-spin"></em> Loading...</td>')
            .fadeOut('slow', function (e) {
                $(this).remove();

                var trs = $('.load-row-layanan tr.n-f-layanan');
                trs.each(function (e, f) {
                    $(trs[e]).find('td.nom-layanan').text(e + 1);
                    $(trs[e]).find('td.select-layanan').find('select').removeAttr('id').attr('id', 'on-select-layanan-' + (e + 1));
                    $(trs[e]).find('td.select-terapis').find('select').removeAttr('id').attr('id', 'on-select-terapis-' + (e + 1));
                });
    ...
    $("table tbody.load-row-paket").delegate('tr > td > em.remove-row-paket', 'click', function (e) {
        $(this).parents("tr").html('<td colspan="3" class="text-center"><em class="fa fa-spinner fa-spin"></em> Loading...</td>')
            .fadeOut('slow', function (e) {
                $(this).remove();

                var trs = $('.load-row-paket tr.n-f-paket');
                trs.each(function (e, f) {
                    $(trs[e]).find('td.nom-paket').text(e + 1);
                    $(trs[e]).find('td.select-paket').find('select').removeAttr('id').attr('id', 'on-select-paket-' + (e + 1));
                    $(trs[e]).find('td.select-paket').find('div#table-th').removeAttr('class').attr('class', 'row hide load-more-paket-' + (e + 1));
                    $(trs[e]).find('td.select-paket').find('tbody#table-td').removeAttr('class').attr('class', 'load-row-paketlayanan-' + (e + 1));
                    $(trs[e]).find('td.select-paket').find('tbody#table-td').find('select').removeAttr('name').attr('name', 'pkt_layanan_terapis[' + (e + 1) + '][]');

                    if ($(trs[e]).find('td.select-paket').find('select#on-select-paket-' + (e + 1)).val() != '') {
                        $(trs[e]).find('td.select-paket').find('div.load-more-paket-' + (e + 1)).removeClass('hide')
                        load_avail_layanan_on_paket('layanan', (e + 1), $(trs[e]).find('td.select-paket').find('select#on-select-paket-' + (e + 1)).val());
                        loadTotal()

                        setTimeout(function () {
                            $(trs[e]).find('td.select-paket').delegate("#on-select-paket-" + (e + 1), 'change', function (ev) {
                                var PktlayId = $(this).val()
                                loadTotal()

                                if (PktlayId) {
                                    load_avail_layanan_on_paket('layanan', (e + 1), PktlayId);
                                } else {
                                    var trps = $("#on-select-onpkt-terapis-" + (e + 1));
                        ...
                        $(trs[e]).find('td.select-paket').find('div.load-more-paket-' + (e + 1)).addClass('hide')
                    }
...
function load_row_paket(idPket) {
    ...
    html += `<td class="select-paket td-height-img">
                ...
                <div class="input-group-sm">
                    <select name="paket[]" form="formRegistrasi" class="select2 form-control input-group-sm" disabled id="on-select-paket-` + numb + `"></select>
                </div>`
    ...
    setTimeout(() => {
        ...
        if (idPket) {
            ...
            load_avail_layanan('paket', numb, $("input[name=id]").data('paket')[idPket - 1]);
            $(".load-more-paket-" + numb).removeClass('hide')
            load_avail_layanan_on_paket('layanan', numb, $("input[name=id]").data('paket')[idPket - 1], $("input[name=id]").data('paket-terapis-' + numb).length);
        } else {
...
function load_row_layanan(idLayanan, idTerapis) {
    ...
    html += `<td class="select-layanan td-height-img">
                <div id="block" class="blocking-loading-row"><em class="fa fa-spinner fa-spin"></em> Loading...</div>
                <div class="input-group-sm">
                    <select ` + (!idLayanan ? ` name="layanan[]" ` : '') + ` form="formRegistrasi" class="select2 form-control input-group-sm" disabled id="on-select-layanan-` + numb + `"></select>
                    ` + (idLayanan ? `<input id="inpt-select-` + numb + `" name="layanan[]" form="formRegistrasi" type="hidden">` : '') + `
                </div>
            </td>`;
    html += `<td class="select-terapis td-height-img">
                <div class="input-group-sm">
                    <select name="terapis[]" form="formRegistrasi" class="select2 form-control input-group-sm" disabled id="on-select-terapis-` + numb + `"></select>
                </div>
            </td>`;
    ...
    setTimeout(() => {
        ...
        if (idLayanan) {
            load_avail_layanan('layanan', numb, $("input[name=id]").data('layanan')[idLayanan - 1]);
            load_avail_layanan('terapis', numb, $("input[name=id]").data('layanan')[idLayanan - 1], $("input[name=id]").data('terapis')[idLayanan - 1]);
            setTimeout(() => {
                $("#on-select-layanan-" + idLayanan).attr('disabled', true)
                $("#inpt-select-" + numb).val($("#on-select-layanan-" + idLayanan).val())
            }, 2000);
        } else {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.length === 0) {
                ...
                $("div#on-input-paketlayananterapis-" + numb + '-lay-' + pID).html(`<input type="hidden" form="formRegistrasi" name="pkt_layanan_terapis[` + numb + `][]" readonly value="0">`);
            } else {
            ...
            for (var i = 0; i < data.length; i++) {
                var selectedTrps = table == 'pegawai' ? ($("input[name=id]").data('paket-terapis-' + numb) ? (data[i].id == $("input[name=id]").data('paket-terapis-' + numb)[pSIDs] ? 'selected' : '') : '') : '';
...
function load_avail_layanan(table, numb, p_id, p_terps) {
    ...
    $.ajax({
        url: base_url + "/registrations/opt/" + table + (table == 'terapis' ? '?layanan=' + pID : ''),
        method: "GET",
        dataType: 'json',
        success: function (data) {
            ...
            for (i = 0; i < data.length; i++) {
                if (table == 'layanan') {
                    html += `<optgroup selected id="` + data[i].id + `" label="` + data[i].nama + `">`;
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += `<td>
                            ...
                            <div class="input-group-sm">
                                <select name="pkt_layanan_terapis[` + numb + `][]" form="formRegistrasi" class="select2 form-control input-group-sm" disabled
...
function form_attribut() {
    ...
    $("em.f-input-an").delegate('select[name=sno_member]', 'change', function (e) {
    ...
    $("div.to-reservasi").delegate("button#hide", "click", function (e) {
        var attr_ = $(this);
        attr_.attr('id', 'show')
        attr_.removeClass('btn-success').trigger('change').addClass('btn-default')
        attr_.find('em').removeClass('fa-check-circle').trigger('change').addClass('fa-question-circle')
        $(".f-reservasi").html("")
    });
...
function select_(table, pkt) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var i;
            for (i = 0; i < data.length; i++) {
                var pegawai = table == 'member' ? data[i].no_member + ` | ` : '';
                var layanan_ = (table == 'layanan' || table == 'paket') ? `data-harga="` + data[i].harga + `"` : '';
                var id_ = (table == 'lokasi') ? data[i].cabang_id : data[i].id;
                html += `<option ` + layanan_ + `value='` + id_ + `'>` + pegawai + data[i].nama + `</option>`;
            }
...
function load_formRight(evv) {
    ...
    $(".display-future").addClass('blocking-content');
    cont.load(base_url + '/registrations/create?form=right', function (e, s, f) {
        if (s == 'error') {
            var fls = 'Gagal memuat form!';
            toastr.error(fls, 'Oops!', {
                timeOut: 2000
...
function getIndoDate(date, hours, row, this_) {
    ...
    var hours__ = (jam__.toString().length == 1 ? '0' + jam__ : jam__) + ':' + (menit__.toString().length == 1 ? '0' + menit__ : menit__);

    var hourss_ = new Date(indTimeNow);
    var jamm__ = hourss_.getHours();
    var menitt__ = hourss_.getMinutes();
    var hourss__ = (jamm__.toString().length == 1 ? '0' + jamm__ : jamm__) + ':' + (menitt__.toString().length == 1 ? '0' + menitt__ : menitt__);

    var kaleNow = new Date().toJSON().slice(0, 10);
    var addJam = hours ? ' <em class="btn btn-' + ((elemnt.toJSON().slice(0, 10) == kaleNow) && (hours__ > hourss__) ? 'info' :
            (elemnt.toJSON().slice(0, 10) < kaleNow ? 'default' : (elemnt.toJSON().slice(0, 10) > kaleNow ? 'info' : (row.print_act == 1 ? 'warning' : 'danger')))) + ' btn-xs text-bold">' +
...
function autoRefreshPage(el, row, this_) {
    ...
    var hours_ = (jam_.toString().length == 1 ? '0' + jam_ : jam_) + ':' + (menit_.toString().length == 1 ? '0' + menit_ : menit_);
...
function data_attribut() {
    ...
    var dTable = $('#data-table-view').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: base_url + "/monitoring/order/json" + dateranges + (dateranges ? "&" : "?") + "statuses=2",
            type: 'GET',
        },
        columns: [{
                data: 'DT_RowIndex',
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return data ? getIndoDate(data, 'jam', row, $('#data-table-view').DataTable()) : '-';
                }
            },
            {
                data: 'total_biaya',
    ...
    var html_ = $('<select id="newFilterStatusOrder" class="form-control input-sm bg-change-select"></select>')
...
function fill_field_daterange(picker) {
    $("input[name='berlaku_dari']").val(picker.startDate.format('DD-MM-YYYY')).attr('data-date-start', picker.startDate.format('YYYY-MM-DD')).trigger('change');
    $("input[name='berlaku_sampai']").val(picker.endDate.format('DD-MM-YYYY')).attr('data-date-end', picker.endDate.format('YYYY-MM-DD')).trigger('change');
    var statusOrder = !$("#newFilterStatusOrder").val() ? '' : $("#newFilterStatusOrder").val();

    $('#data-table-view').DataTable().ajax.url(base_url +
        "/monitoring/order/json?starts=" + picker.startDate.format('YYYY-MM-DD') +
        "&ends=" + picker.endDate.format('YYYY-MM-DD') + "&statuses=" + statusOrder).load();
}

function getStatus_field_daterange() {
    var dateStart = $(".tinggi-filter-range-date").find("input[name=berlaku_dari]")[0].dataset.dateStart;
    var dateEnd = $(".tinggi-filter-range-date").find("input[name=berlaku_sampai]")[0].dataset.dateEnd;
    var statusOrder = !$("#newFilterStatusOrder").val() ? '' : $("#newFilterStatusOrder").val();

    $('#data-table-view').DataTable().ajax.url(base_url +
        "/monitoring/order/json?starts=" + (dateStart ? dateStart : '') +
        "&ends=" + (dateEnd ? dateEnd : '') + "&statuses=" + statusOrder).load();
}
...
function remove_field_daterange() {
    ...
    $('#data-table-view').DataTable().ajax.url(base_url +
        "/monitoring/order/json?statuses=" + statusOrder).load();
}
...
$(document).ready(function () {
    $("input[name=berlaku_dari]").val(rangeDate(0)).attr('data-date-start', rangeDate(0, 'back'))
    $("input[name=berlaku_sampai]").val(rangeDate(1)).attr('data-date-end', rangeDate(1, 'back'))
