...
function load_formProfile() {
    ...
    cont.load(base_url + "/profiles/create", function (e, s, f) {
        if (s == "error") {
            ...
            $(this).html(
                '<div class="box-body"><em class="fa fa-warning"></em> ' +
                    fls +
                    "</div>"
            );
        } else {
        ...
        if (input.files && input.files[0]) {
            ...
            if (
                ...
                if (imageSize > 0.5) {
                    ...
                    toastr.warning(
                        "Ukuran foto tidak boleh lebih dari 500 KB !",
                        "Oops!",
                        {
                            timeOut: 2000,
                        }
                    ...
                    fileReader.onload = function (e) {
                        ...
                        image.onload = function () {
                            ...
                            if (inHeight > max_height && inWidth > max_width) {
                                $("input[name=photo]").val("");
                                toastr.warning(
                                    "Ukuran foto tidak boleh lebih dari 340px X 310px !",
                                    "Oops!",
                                    {
                                        timeOut: 2000,
                                    }
                ...
                $("input[name=photo]").val("");
                toastr.warning(
                    "Format foto yang diizinkan hanya jpg/png !",
                    "Oops!",
                    {
                        timeOut: 2000,
                    }
            ...
            success: function (data) {
                switch (data.cd) {
                    case 200:
                        ...
                        $(".user-header")
                            .find("img")
                            .attr("src", base_url + images_);
                        $(".load-detail")
                            .find(".profile-username")
                            .text(data.data.nama);
                        $(".load-detail")
                            .find("p.det-mail")
                            .html("<small>" + data.data.email + "</small>");
                        $(".load-detail")
                            .find("p.det-jabatan")
                            .html(
