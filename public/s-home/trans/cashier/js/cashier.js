        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                setTimeout(function () {
                    ...
                    $(".load-form-left-info-transaksi").html(contInfoTrans(inputId))
                ...
                setTimeout(() => {
                    ...
                    for (var rPd = 1; rPd <= inputId.data('produk').length; rPd++) {
                    ...
                    $(".load-form-pembayaran").append('<div class="price-layanan-unique"></div>')
                }, 2000);
            },
            error: function () {
...
function form_attribut_modal(ev) {
    $(".modal-title").html('<em class="fa fa-pencil-square-o"></em> Form Pembayaran')
    load_formLeft();
    load_formRightOrder();
    load_formRight(ev);
}
...
function rePrint(events, dataTrans, onsave) {
    ...
    printer.open().then(function () {
        ...
        printer.align('center').text("--------------------------------").text('PRINT')
            .feed(1)
    ...
    var dTable = $('#data-table-view').DataTable({
        ...
        drawCallback: function (settings) {
            ...
            api.column(groupColumn, {
                ...
                if (last !== group) {
                    $(rows).eq(i).before(
                        '<tr class="group text-center" style="color:#fff !important; background-color:#7d8084 !important;"><td colspan="10"><strong>' + group + ' ( ' + api.column(2).data()[i] + ' ) ' + '</strong></td></tr>'
                    );
...
function fill_field_daterange(picker) {
    ...
    $('#data-table-view').DataTable().ajax.url(base_url +
        "/cashiers/json?starts=" + picker.startDate.format('YYYY-MM-DD') +
...
function loadTotal(idLayanan) {
    ...
    setTimeout(() => {
        ...
        var RealTagihan = total_harga_paket + total_harga_layanan + total_harga_produk;
        ...
        if (value) {
            ...
            var addOn = param == '%' ? ` ( -` + nominal + `% ) ` : ` ( -Rp. ` + convertRupiah(nominal) + ` ) `
