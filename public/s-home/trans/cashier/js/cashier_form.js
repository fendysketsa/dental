    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < data.length; i++) {
                html += `<option value='` + data[i].id + `'>` + data[i].no_member + ` | ` + data[i].nama + `</option>`;
            }
...
function select_(table) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var i;
            for (i = 0; i < data.length; i++) {
                var percents = table == 'voucher' ? `data-diskon="` + data[i].diskon + `"` : '';
                var nomDiskon = table == 'diskon' ? `data-param="` + data[i].param + `" data-nominal="` + data[i].nominal + `"` : '';
                html += `<option ` + percents + ` ` + nomDiskon + ` value='` + data[i].id + `'>` + data[i].nama + `</option>`;
            }
...
function f_member(data) {
    ...
    html += `<div class="form-group">
                ...
                <div class="input-group input-group-sm date">
                    ...
                    <input type="text" name="nama" value="` + (data ? data.nama : '') + `" class="form-control input-sm" placeholder="Nama..." required>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="email" name="email" value="` + (data ? data.email : '') + `" class="form-control input-sm" placeholder="Email..." required>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="input-group input-group-sm date">
                            ...
                            <input type="text" name="telepon" value="` + (data ? data.telepon : '') + `" class="form-control input-sm" placeholder="Telepon..." required>
                        </div>
                    </div>
                </div>
            </div>
            <div class="form-group">
                <label>Alamat: <em class="text-danger">*</em></label>
                    ...
                    <textarea name="alamat" cols="15" rows="4" class="form-control add-style input-sm" style="height:50px;" placeholder="Alamat...">` + (data ? data.alamat : '') + `</textarea>
                </div>
            </div>`;
    return html;
}
        ...
        success: function (data) {
            ...
            if (id_ && $("select#on-prd-" + numb).val() == id_) {
                setTimeout(() => {
                    ...
                    $("input#on-prd-value-" + numb).val($('input[name=id]').data('produk')[numb - 1]);
                    $("em#prc-" + numb).text($('input[name=id]').data('produk-harga')[numb - 1]);
                    $("input#qty-" + numb).removeAttr('disabled').val($('input[name=id]').data('produk-jumlah')[numb - 1]);
...
function form_attribut() {
    ...
    $("em.f-input-an").delegate('select[name=no_member]', 'change', function (e) {
...
function form_attribut_right() {
    ...
    $("table tbody").delegate('tr > td > em.delete-rows', 'click', function (e) {
        var valueBeforeDelete = $(this).parents('tr').find('em.subtotal').text();
        $(".total-belanja").text(parseInt($(".total-belanja").text()) - parseInt(valueBeforeDelete));
        ...
        trs.each(function (e, f) {
            ...
            $(trs[e]).find('td.select').find('input').removeAttr('id').attr('id', 'on-prd-value-' + (e + 1));
            $(trs[e]).find('td.prc').find('em').removeAttr('id').attr('id', 'prc-' + (e + 1));
        ...
        var total_temp = parseInt($("input[name=id]").data('total-biaya')) + parseInt(total_all);
        ...
        if ($("#voucher").val()) {
            ...
            $(".total-belanja").text(parseInt(total_temp) + parseInt($("#voucher").attr('temp-diskon') ? $("#voucher").attr('temp-diskon') : 0));
    ...
    $("table tbody").delegate('tr > td', 'input change', function (e) {
        ...
        var total_temp = parseInt($("input[name=id]").data('total-biaya')) + parseInt($(".total-belanja-produk").text() ? $(".total-belanja-produk").text() : 0);
        var voucherInUse = $("#voucher").find('option:selected').data('diskon');
        var AfDisk = 0;
    ...
    $("#cara_bayar").on('change', function (e) {
        ...
        if (Id) {
            if (Id == 2) {
                ...
                $("input[name=bayar]").removeAttr("readonly").removeAttr('disabled');
                $("#content-bayar").html('')
                $("#content-methode-bayar").html('')
            }
        } else {
            $("input[name=bayar]").attr("readonly", true).attr('disabled', true);
            $("#content-bayar").html('')
            $("#content-methode-bayar").html('')
        }
    });

    $("#content-methode-bayar").delegate('#cara_bayar_select', 'change', function () {
        var Id = $(this).val();
    ...
    $("#voucher").on('change', function () {
        ...
        if (Elm.val()) {
            var AfterDiskon = (parseInt(Elm.find('option:selected').data('diskon')) * parseInt(total_all_belanja.text())) / 100;
            total_all_belanja.text(parseInt(total_all_belanja.text()) - parseInt(AfterDiskon));
            Elm.attr('temp-diskon', AfterDiskon);
        } else {
            total_all_belanja.text(parseInt(total_all_belanja.text()) + parseInt(Elm.attr('temp-diskon') ? Elm.attr('temp-diskon') : 0));
    ...
    $("input[name=bayar]").on('input', function (e) {
        ...
        if (hitungan >= 0) {
            ...
            $("button[type=submit]").removeAttr('disabled').attr('form', 'formKasir');
        } else {
...
function show_on_form_right_kasir(fals, f_left) {
    ...
    trs.each(function (e, f) {
        ...
        var field_harga = $(trs[e]).find('select#on-prd-' + (e + 1) + ' option:selected');
        var harga = (!member_or_no && !f_left) ? field_harga.data('harga') : field_harga.data('harga-member');
        ...
        if (!cek) {
            ...
            $("#add-row").removeClass('disabled')
            $("#add-row").addClass('add-row')
            $(trs[e]).find('select#on-prd-' + (e + 1)).prop("disabled", true);
            if (fals == 0) {
        ...
        if (cek === value.val() && cek !== '') {
            ...
            setTimeout(function () {
                ...
                setTimeout(() => {
                    ...
                    $(".total-belanja").text(parseInt($("input[name=id]").data('total-biaya')) + parseInt($(".total-belanja-produk").text()));
                }, 0);
            }, 0);
        }
    }
...
function load_row_paket(idPket) {
    ...
    html += `<td class="select-paket td-height-img">
                ...
                <div class="input-group-sm">
                    <select name="paket[]" form="formKasir" class="select2 form-control input-group-sm" disabled id="on-select-paket-` + numb + `"></select>
                </div>`
...
function load_row_produk(idProduk) {
    ...
    setTimeout(() => {
        ...
        if (idProduk) {
            load_avail_produk(numb, $("input[name=id]").data('produk')[idProduk - 1]);
        } else {
            load_avail_produk(numb);
        }
