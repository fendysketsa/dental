...
function input_member() {
    ...
    html += `<input type="text" name="ino_member" readonly value="` + DatAuto + `" class="form-control input-sm no-border" placeholder="No Member..." form="formRegistrasi">`;
    return html;
}
...
function f_member(data) {
    ...
    html += `<div class="form-group">
                ...
                <div class="input-group input-group-sm date">
                    ...
                    <input type="text" name="nama" value="` + (data ? data.nama : '') + `" class="form-control" placeholder="Nama..." form="formRegistrasi">
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="email" ` + (data ? (data.email ? "readonly" : '') : '') + ` name="email" value="` + (data ? (data.email ? data.email : '') : '') + `" class="form-control" placeholder="Email..." form="formRegistrasi">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="input-group input-group-sm date">
                            ...
                            <input type="text" name="telepon" value="` + (data ? data.telepon : '') + `" class="form-control" placeholder="Telepon..." form="formRegistrasi">
                        </div>
                    </div>
                </div>
            </div>
            <div class="form-group input-group-sm">
                <label>Alamat:</label>
                    ...
                    <textarea name="alamat" cols="15" rows="4" class="form-control input-sm add-style" form="formRegistrasi" placeholder="Alamat...">` + (data ? data.alamat : '') + `</textarea>
                </div>
            </div>`;
    return html;
}
...
function form_attribut_right() {
    ...
    $("select.f-layanan").on('change', function (e) {
        ...
        var harga_paket = $("select[name=paket]").find('option:selected').data('harga');
        ...
        $(this).parents("tr").html('<td colspan="4" class="text-center"><em class="fa fa-spinner fa-spin"></em> Loading...</td>')
            .fadeOut('slow', function (e) {
                ...
                trs.each(function (e, f) {
                    ...
                    $(trs[e]).find('td.select-ruangan').find('select').removeAttr('id').attr('id', 'on-select-ruangan-' + (e + 1));
                });
...
function load_row_layanan() {
    ...
    html += `<td class="select-layanan td-height-img">
                <div id="block" class="blocking-loading-row"><em class="fa fa-spinner fa-spin"></em> Loading...</div>
                <div class="input-group-sm">
                    <select name="layanan[]" form="formRegistrasi" class="select2 form-control input-group-sm" disabled id="on-select-layanan-` + numb + `"></select>
                </div>
            </td>`;
    html += `<td class="select-ruangan td-height-img">
                <div class="input-group-sm">
                    <select name="ruangan[]" form="formRegistrasi" class="select2 form-control input-group-sm" disabled id="on-select-ruangan-` + numb + `"></select>
                </div>
            </td>`;
...
function load_avail_playanant(table, numb, layanan) {
    ...
    $.ajax({
        url: base_url + "/registrations/opt-ruangan/" + table + '?layanan=' + pID,
        method: "GET",
        dataType: 'json',
        success: function (data) {

            if (data.length === 0) {
                $("select#on-select-paketlayananruangan-" + numb + '-lay-' + pID).attr('disabled', true).removeAttr('name,form');
                $("div#on-input-paketlayananruangan-" + numb + '-lay-' + pID).html(`<input type="hidden" form="formRegistrasi" name="pkt_layanan_ruangan[` + (numb - 1) + `][]" readonly value="0">`);
            } else {
                $("select#on-select-paketlayananruangan-" + numb + '-lay-' + pID).removeAttr('disabled');
            }
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                $("select#on-select-paketlayananruangan-" + numb + '-lay-' + pID).html(html);
            }

            $("select#on-select-paketlayananruangan-" + numb + '-lay-' + pID).select2({
                placeholder: "Please select!",
                allowClear: true,
                theme: "bootstrap"
            });
...
function load_avail_layanan(table, numb, p_id) {
    ...
    var urlAdd = (resTag && resJam && resLoc ? '&tgl_res=' + resTag + '&jam_res=' + resJam + '&loc_res=' + resLoc : '');

    $.ajax({
        url: base_url + "/registrations/opt/" + table + (table == 'ruangan' ? '?layanan=' + pID + urlAdd : ''),
        method: "GET",
        dataType: 'json',
        success: function (data) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                if (table == 'layanan') {
                    ...
                    for (var ii = 0; ii < data[i].data.length; ii++) {
                        ...
                        html += `<option ` + harga_ + ` alt="` + data[i].nama + `" value='` + data[i].data[ii].id + `'>` + data[i].data[ii].nama + `</option>`;
                    }
                    html += `</optgroup>`;
                }
                ...
                if (table != 'layanan') {
                    ...
                    var AvaliableTrps = table == 'paket' ? '' : (data[i].on_work == 'true' ? 'disabled' : (data[i].available == 'true' ? '' : ''));
                    html += `<option ` + AvaliableTrps + ` ` + harga + ` value='` + data[i].id + `'>` + data[i].nama + `</option>`;
                }
            }
    ...
    $.ajax({
        ...
        success: function (data) {
            for (var i = 0; i < data.length; i++) {
                ...
                html += `<td>
                            <div id="on-input-paketlayananruangan-` + numb + `-lay-` + data[i].id + `"></div>
                            <div class="input-group-sm">
                                <select name="pkt_layanan_ruangan[` + (numb - 1) + `][]" form="formRegistrasi" class="select2 form-control input-group-sm" disabled
                                    id="on-select-paketlayananruangan-` + numb + `-lay-` + data[i].id + `"></select>
                            </div>
                        </td>`;
                html += `</tr>`;
...
function form_attribut() {
    ...
    $("em.f-input-an").delegate('select[name=sno_member]', 'change', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("div.date").delegate("div.on-edit-true", 'click', function (e) {
                    $(this).closest('div.date').find('input').removeAttr('readonly')
                });
            }
        });
    });
    ...
    $("div.change-to-field").delegate('input[name=ino_member]', 'input', function (e) {
    ...
    $("div.to-reservasi").delegate("button#show", "click", function (e) {
        ...
        $(".to-change-it").delegate('#edit', 'click', function () {
            ...
            $(this).removeClass('fa-edit').addClass('fa-clock-o').attr('id', 'no-edit');
        });
        $(".to-change-it").delegate("#no-edit", 'click', function () {
            $("input[name=jam_reservasi]").attr('readonly', true);
            $(this).removeClass('fa-clock-o').addClass('fa-edit').attr('id', 'edit');
        });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                html += `<option ` + layanan_ + `value='` + data[i].id + `'>` + pegawai + data[i].nama + `</option>`;
                ...
                printer
                    ...
                    .align('left').text(
                        sprintf('%s', ' 1 X ' + dataTrans.layanan[lyn].layanan.substring(0, 25) + '/' +
                            (dataTrans.layanan[lyn].ruangan ? dataTrans.layanan[lyn].ruangan.substring(0, 8) : '-')
                        )
                    );
            }
        }
            ...
            for (var pketl = 0; pketl < dataTrans.paket.length; pketl++) {
                ...
                printer.align('left').text(
                    ' - ' + sprintf('%s', dataTrans.paket[pketl].layanan.substring(0, 25) + '/' +
                        (dataTrans.paket[pketl].ruangan ? dataTrans.paket[pketl].ruangan.substring(0, 8) : '-')
                    )
                );
            }
        }
